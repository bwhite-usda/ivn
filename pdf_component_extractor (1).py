...
def main():
    ...
    # --- Build output path (same directory as PDF) ---
    timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M")
    suggested_name = safe_filename(pdf_path.stem, ".xlsx", timestamp)
    output_dir = pdf_path.parent
    output_xlsx = output_dir / suggested_name

    # Allow user override (directory only)
    print(f"\nDefault output file will be:\n{output_xlsx}")
    alt_dir = input("\nEnter alternative output directory (or press Enter to accept default): ").strip()
    if alt_dir:
        alt_dir_path = Path(alt_dir).expanduser()
        try:
            alt_dir_path.mkdir(parents=True, exist_ok=True)
            output_xlsx = alt_dir_path / suggested_name
        except Exception as e:
            print(f"Could not use alternative directory ({e}); using default.")
    ...
    try:
        ...
        ws = wb.active
        ws.title = safe_title
        headers = ["Source Name", "Component Name", "Component Description", "Component URL"]
        ws.append(headers)
